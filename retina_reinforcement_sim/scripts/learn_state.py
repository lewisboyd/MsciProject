...
if __name__ == '__main__':
    ...
    MODEL_FOLDER = (os.path.dirname(
        os.path.realpath(__file__)) + "/pendulum/state_dict/pixel_supervised")
    ...
    optimiser = torch.Adam(net.parameters())
    ...
    for epoch in range(1, max_epoch+1):
        ...
        print "Epoch: %3d/%3d  Train Loss: %0.2f Test Loss: %0.2f" % (
            ep, max_epoch, train_loss, test_loss)
        ...
        if (epoch % SAVE_FREQ == 0 or epoch == max_epoch):
            torch.save(net.state_dict(), MODEL_FOLDER + "net_" + epoch)
